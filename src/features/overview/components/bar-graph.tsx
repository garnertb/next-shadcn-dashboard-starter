...
export const description =
  'An interactive bar chart showing Copilot suggestions';

// Transform the Copilot data for the last 30 days
const chartData = mockCopilotUsageMetrics.map((metric) => ({
  date: metric.day,
  suggestions: metric.total_suggestions_count,
  acceptances: metric.total_acceptances_count
}));
    ...
    <Card className='@container/card !pt-3'>
      <CardHeader className='flex flex-col items-stretch space-y-0 border-b !p-0 sm:flex-row'>
        <div className='flex flex-1 flex-col justify-center gap-1 px-6 !py-0'>
          <CardTitle>Copilot Suggestions Trend</CardTitle>
          <CardDescription>
            <span className='hidden @[540px]/card:block'>
              Daily suggestions and acceptances for the last 30 days
            </span>
            <span className='@[540px]/card:hidden'>Last 30 days</span>
          </CardDescription>
        </div>
        <div className='flex'>
          {['suggestions', 'acceptances'].map((key) => {
